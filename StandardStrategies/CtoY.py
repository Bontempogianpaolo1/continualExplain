...
import torch
from nets.model import base, triLayer


class CtoY(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        attribute = torch.stack(attributes, dim=1).float()
        preds = self.model(attribute)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        attribute=torch.stack(attribute,dim=1).float()
        preds = self.model(attribute)
        loss = self.loss_module(preds, label)
        acc = (preds.argmax(dim=-1) == label).float().mean()
