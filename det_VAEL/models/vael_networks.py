...
class DetMNISTPairsEncoder(nn.Module):
    ...
    def __init__(self, img_channels=1, hidden_channels=32, latent_dim=8, dropout=0.5):
        ...
        # self.enc_block_1 = nn.Linear(self.img_channels,
        #                  256)  # hidden_channels x 14 x 28

        # self.enc_block_2 = nn.Linear(256, 128)

        # self.enc_block_3 = nn.Linear(128,
        #                             4 * self.hidden_channels * self.unflatten_dim[0] * self.unflatten_dim[1] )

        self.flatten = Flatten()
#        print(self.latent_dim, self.unflatten_dim[0] * self.unflatten_dim[1] )
        self.dense_mu = nn.Linear(
            in_features=4 * self.hidden_channels * self.unflatten_dim[0] * self.unflatten_dim[1],
            out_features=self.latent_dim)
