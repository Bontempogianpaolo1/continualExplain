...
class DeepProblogModel(nn.Module):
    ...
    def __init__(self, encoder, mlp, latent_dims, model_dict, dropout=None, is_train=True, device='cpu'):
        ...
        self.mlp = mlp
        self.dropout = dropout
        self.latent_dim_sym = latent_dims
        self.model_dict = model_dict  # Dictionary of pre-compiled ProbLog models
        self.is_train = is_train
        self.device = device
...
class MNISTPairsVAELModel(VAELModel):
    ...
    def problog_inference(self, facts_probs, labels=None, query=None):
        ...
        """
        if query == None:
            query = int(labels[0][-1])  # Query of the current batch
...
class MNISTPairsDeepProblogModel(DeepProblogModel):

    def __init__(self, encoder, 
                 mlp, latent_dims, model_dict,
                 w_q, dropout=None, is_train=True, device=False):
        super(MNISTPairsDeepProblogModel, self).__init__(encoder=encoder, mlp=mlp, latent_dims=latent_dims,
                                                  model_dict=model_dict, dropout=dropout, is_train=is_train,
                                                  device=device)
