...
def train_PLEnc(model, optimizer, n_epochs, train_set, val_set, folder, early_stopping_info=None, run_ID='_',
                query=True, recon_w=1, kl_w=1, query_w=1, sup_w=0,
                rec_loss='MSE', train_batch_size=32, val_batch_size=32):
    Path(os.path.join(folder, run_ID)).mkdir(parents=True, exist_ok=True)
    if sup_w > 0:
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for batch, (data, labels) in enumerate(train_set, 1):
            ...
            # Forward pass
            mu, add_prob = model(data.permute(1, 0, 2, 3), labels)
            loss, recon_loss, gauss_kl_div, queryBCE, labelBCE = det_loss_function(data.permute(1, 0, 2, 3),
                                                                               mu,
                                                                               add_prob,
                                                                               labels=labels,
                                                                               query=query,
